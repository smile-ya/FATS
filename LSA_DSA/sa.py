...
def _get_train_target_ats(model, x_train, x_target, target_name, layer_names, args):
    ...
    if os.path.exists(saved_train_path[0]):
        print(utils.infog("Found saved {} ATs, skip serving".format("train")))
        ...
        print(utils.infog("train ATs is saved at " + saved_train_path[0]))

    saved_target_path = _get_saved_path(
        args.save_path, args.d, target_name, layer_names
    )
    if os.path.exists(saved_target_path[0]):
        print(utils.infog("Found saved {} ATs, skip serving").format(target_name))
        ...
        print(utils.infog(target_name + " ATs is saved at " + saved_target_path[0]))
...
def fetch_dsa(model, x_train, x_target, target_name, layer_names, args):
    ...
    if os.path.exists("LSA_DSA/dsa/"+target_name+args.d+'_dsaats.npy'):
        print(utils.infog("Found saved {} DSAs, skip serving").format(target_name))
        # In case target_ats is stored in a disk
        dsa = np.load("LSA_DSA/dsa/"+target_name+args.d+'_dsaats.npy')
        return list(dsa)
    ...
    np.save("LSA_DSA/dsa/"+target_name+args.d+'_dsaats.npy',dsa)
    ...
    if args.is_classification:
        ...
        for label in tqdm(range(args.num_classes), desc="kde"):
            ...
            if refined_ats.shape[0] == 0:
                print(
                    utils.warn("ats were removed by threshold {}".format(args.var_threshold))
                )
                break
        ...
        refined_ats = np.delete(refined_ats, removed_cols, axis=0)
        if refined_ats.shape[0] == 0:
            print(utils.warn("ats were removed by threshold {}".format(args.var_threshold)))
        kdes = [gaussian_kde(refined_ats)]

    print(utils.infog("The number of removed columns: {}".format(len(removed_cols))))
...
def fetch_lsa(model, x_train, x_target, target_name, layer_names, args):
    ...
    if os.path.exists("LSA_DSA/lsa/"+target_name+args.d+'_lsa.npy'):
        print(utils.infog("Found saved {} LSAs, skip serving").format(target_name))
        # In case target_ats is stored in a disk
        lsa = np.load("LSA_DSA/lsa/"+target_name+args.d+'_lsa.npy')
        return list(lsa)
    ...
    if args.is_classification:
        ...
        for at in tqdm(target_ats):
            lsa.append(_get_lsa(kde, at, removed_cols))
    np.save("LSA_DSA/lsa/"+target_name+args.d+'_lsa.npy',lsa)
    return lsa

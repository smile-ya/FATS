...
## deepgini
def deepgini(dataset, model, selectsize, x_test, y_test):
...
# 返回MCP优化版的采样用例
def select_my_MCP(dataset, model, selectsize, x_test, y_test):
    if dataset == 'mnist' or dataset == 'fashion_mnist':
        x = np.zeros((selectsize, 28, 28, 1))
        y = np.zeros((selectsize,))
    elif dataset == 'cifar10' or dataset == 'svhn':
...
def retrain(begin,res, train_act_layers, model, args, layer_names,ranknet_model,x_train, x_test, y_test, selectsize=100,
    ...
    if measure == 'deepgini':
        x_select, y_select, rank_list = deepgini(args.d, model, selectsize, x_test, y_test)

    if measure == 'SRS':
        x_select, y_select, rank_list = select_rondom(args.d, selectsize, x_test, y_test)
    if measure == "MCP":
        x_select, y_select, rank_list = select_my_MCP(args.d, model, selectsize, x_test, y_test)
    if measure == 'LSA':
    ...
    if measure == 'AAL':
        path = 'AAL/CVPR13/' + args.d + '_finalResults/' + args.d + '_' + attack + "_compound8_result.csv"
        csv_data = pd.read_csv(path, header=None)
        target_lst = []
        for i in range(len(csv_data.values.T)):
            target_lst.append(csv_data.values.T[i])
    if measure == 'CES':
        indexlst = condition.conditional_sample(model, x_test, selectsize)
        rank_list = indexlst
        x_select, y_select = select_from_index(args.d, selectsize, x_test, indexlst, y_test)
    elif measure not in ['SRS', 'MCP', 'FATS',  'deepgini', 'CES']:
...
# 验证指标1：APFD: 平均故障检测率， 用于评估测试集优先级技术的有效性
def apfd(model, x_test, y_test, rank_list):  # 参数均为numpy.array类型
    y_test = np_utils.to_categorical(y_test, 10)

    predict_pro = model.predict(x_test)
    print(predict_pro.shape)
    print(y_test.shape)

    predict_maxIndex = np.argmax(predict_pro, axis=1)
    y_testIndex = np.argmax(y_test, axis=1)
    m = np.sum(predict_maxIndex != y_testIndex)  # 两下标列表中不相同的元素个数，表示错误用例数目

    n = y_test.shape[0]  # 测试用例总数目

    index = np.arange(0, n)
    array = index[predict_maxIndex != y_testIndex]  # 找到两个数组不相等元素的索引位置,返回的是一个numpy.array
    test_fault_index = []
    for item in array:
        index = np.where(rank_list == item)
        test_fault_index.append(index[0][0])
    fault_indexSum = sum(test_fault_index)
    # print(test_fault_index)
    # print(fault_indexSum)
    return 1 - float(fault_indexSum) / (n * m) + 1. / (2 * n)

#验证指标2：FaultDetectionRatio 错误检测率
def fdr(model, allFault_number, x_select, y_select):  # 参数均为numpy.array类型

    predict_pro = model.predict(x_select)
    predict_maxIndex = np.argmax(predict_pro, axis=1)
    y_testIndex = np.argmax(y_select, axis=1)
    m = np.sum(predict_maxIndex != y_testIndex)  # 两下标列表中不相同的元素个数，表示选择的样本中错误用例的数目
    return m / allFault_number

#验证指标2：FaultDetectionRatio 错误检测率 ,验证指标3：class_fault 类别错误检测率
def fdr_old(model, allFault_number,allclass_score, x_select, y_select,):  # 参数均为numpy.array类型

    predict_pro = model.predict(x_select)
    predict_maxIndex = np.argmax(predict_pro, axis=1)
    y_testIndex = np.argmax(y_select, axis=1)
    m = np.sum(predict_maxIndex != y_testIndex)  # 两下标列表中不相同的元素个数，表示选择的样本中错误用例的总数目
    fdr = m / allFault_number

    classFault_number = [0 for i in range(10)]
    length = 0
    class_fault = 0
    for i in range(len(y_testIndex)):
        if predict_maxIndex[i] != y_testIndex[i]:
            classFault_number[y_testIndex[i]] += 1
            length += 1

    for i in range(len(classFault_number)):
        class_fault += ( classFault_number[i] / m ) * allclass_score[i]

    print("###############计算的总错误用例数是否正确###################")
    print(length == m)

    return fdr,class_fault

def allclass_fault_score(model, x_test,y_test):
    predict_pro = model.predict(x_test)
    predict_maxIndex = np.argmax(predict_pro, axis=1)
    allFault_number = np.sum(predict_maxIndex != y_test)  # 两下标列表中不相同的元素个数，表示测试集总的错误用例数目

    allclassFault_number = [ 0 for i in range(10)]
    length = 0
    for i in range(len(y_test)):
        if predict_maxIndex[i] != y_test[i]:
            allclassFault_number[y_test[i]] += 1
            length += 1

    allclass_score = [0 for i in range(10)]
    for i in range(len(allclassFault_number)):
        allclass_score[i] = allclassFault_number[i] / allFault_number #存储总的错误用例数中，每个类别下的错误用例数的占比


    print("###############计算的总错误用例数是否正确###################")
    print(length==allFault_number)
    return allFault_number,allclass_score
...
if __name__ == "__main__":
    ...
    for operator in operators:
        ...
        # allFault_number,allclass_score = allclass_fault_score(model, x_test, y_test)
        predict_pro = model.predict(x_test)
        predict_maxIndex = np.argmax(predict_pro, axis=1)
        allFault_number = np.sum(predict_maxIndex != y_test)  # 两下标列表中不相同的元素个数，表示测试集总的错误用例数目
        print("---------------allFault_number-------------------")
        print(allFault_number)
        for measure in baselines:
            ...
            for selectsize in selectsizelist:
                ...
                # #验证指标2：FaultDetectionRatio 错误检测率
                FDR = fdr(model, allFault_number, x_select, y_select)
                print("FDR" + str(FDR))
                print("用时为:" + str(Time))
                result_to_write += 'selectsize:' + str(selectsize) + ','
                result_to_write += 'Time:' + str(round(Time, 4)) + 's,'
                result_to_write += 'FDR:' + str(round(FDR, 4)) + ','

...
def saliency_map_method(sess,dataset, model, X, Y, theta, gamma, clip_min=None,
                        clip_max=None):
    """
    TODO
    :param sess:
    ...
    for i in tqdm(range(len(X))):
        ...
        npyfile = os.path.join(PATH_DATA + dataset, 'Adv_' + dataset + '_jsma_%s.npy' % str(i))
        if os.path.exists(npyfile):
            ...
            #if (i+1)%100==0:
            np.save(os.path.join(PATH_DATA + dataset, 'Adv_' + dataset + '_jsma_%s.npy' % str(i)), X_adv[i])

...
import densenet, RCNN
# CLIP_MIN = 0.0
# CLIP_MAX = 1.0
# CLIP_MIN = -0.5
CLIP_MAX = 0.5
PATH_DATA = "./svhn/svhn_data"
...
def get_data(dataset='mnist'):
    ...
    :return:
    """
    assert dataset in ['mnist', 'cifar10', 'cifar100', 'fashion_mnist', 'svhn'], \
        "dataset parameter must be either 'mnist', 'cifar10', 'cifar100', 'fashion_mnist', 'svhn' "
    if dataset == 'mnist':
        ...
        # print("os.path")
        # print(os.path.abspath('.')) #返回脚本所在的绝对路径/home/tfq/project/lili/MCP-master/dataset/attack
        # print(os.path.isfile(os.path.join(os.path.abspath('..'),"svhn/svhn_data/svhn_train.mat")))

        if not os.path.isfile(os.path.join(os.path.abspath('..'),"svhn/svhn_data/svhn_train.mat")):
            print('Downloading SVHN train set...')
            call(
                "curl -o ./svhn/svhn_data/svhn_train.mat"
                "http://ufldl.stanford.edu/housenumbers/train_32x32.mat",
                shell=True
            )
        if not os.path.isfile(os.path.join(os.path.abspath('..'),"svhn/svhn_data/svhn_test.mat")):
            print('Downloading SVHN test set...')
            call(
                "curl -o ./svhn/svhn_data/svhn_test.mat"
                "http://ufldl.stanford.edu/housenumbers/test_32x32.mat",
                shell=True
            )
        train = sio.loadmat(os.path.join(os.path.abspath('..'),"svhn/svhn_data/svhn_train.mat"))
        test = sio.loadmat(os.path.join(os.path.abspath('..'),"svhn/svhn_data/svhn_test.mat"))
...
def get_model(dataset='mnist', softmax=True):
    ...
    if dataset == 'mnist':
        ...
        # ]
        layers = [
            Conv2D(6, (5 , 5), padding='valid', input_shape=(28, 28, 1)),  # 0
            Activation('relu'),  # 1
            # BatchNormalization(), # 2
            ...
            MaxPooling2D(pool_size=(2, 2)),  # 6
            Flatten(),  # 8
            Dense(120,activation='relu'),  # 9
            Dense(84,activation='relu'),  # 10
            Dropout(0.5),
            Dense(10), # 11
        ]
        model = Sequential()
        ...
        #     Dense(10),  # 31
        # ]
         model = densenet.create_dense_net(10,softmax,(32, 32, 3), 40, 3, 12, 16,dropout_rate=0)
    ...
    # if softmax:
    #     model.add(Activation('softmax'))
        model = RCNN.get_model(softmax)
    return model
def get_model_old(dataset='mnist', softmax=True):
    """
    Takes in a parameter indicating which model type to use ('mnist',
    'cifar' or 'svhn') and returns the appropriate Keras model.
    :param dataset: A string indicating which dataset we are building

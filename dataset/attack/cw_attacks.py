...
import os
from dataset.util import lid_adv_term
...
PATH_DATA = "../mutation/adv/"
class CarliniL2:
    def __init__(self, sess, model,dataset, image_size, num_channels, num_labels, batch_size=100,
    ...
    def attack(self, X, Y):
        ...
        for i in tqdm(range(0, X.shape[0], self.batch_size)):
            ...
            filename = os.path.join(PATH_DATA + self.dataset, 'Adv_%s_cw-l2_%s.npy' % (self.dataset,str(i)))
    ...
    def attack_batch(self, imgs, labs):
        ...
        # imgs= imgs - 0.2 # cifar：0.2 mnist：0.5 svhn：0.5
        # convert to tanh-space
        # print("---------imgs----------")
        # print(type(imgs))
        # print(imgs)
        # return
        imgs = np.arctanh(imgs * 0.9) #cifar10:0.9 ,mnist&&svhn:1.999999
        # imgs = np.arctanh(imgs * 1.999999)  #np.arctanh 对数组中的每一个元素求其反正切值，它是正切函数的反函数，如果y=tan(x),
                                            # 那么x=arctan(y)
